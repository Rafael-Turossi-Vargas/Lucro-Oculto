    ...
    <div className="min-h-screen" style={{ background: "#0F1117" }}>
      ...
      <main className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 pt-28 pb-20">
        ...
        <div className="mb-10">
          ...
          <p className="text-sm" style={{ color: "#4B4F6A" }}>
        ...
        <div
          ...
          style={{ color: "#8B8FA8", lineHeight: "1.8" }}
        >
          {children}
        </div>
      </main>
...
export function LegalSection({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <section className="space-y-3">
      <h2 className="text-lg font-bold" style={{ color: "#F4F4F5" }}>{title}</h2>
      <div className="text-sm leading-relaxed" style={{ color: "#8B8FA8" }}>{children}</div>
    </section>
  )
}
...
export function LegalHighlight({ children }: { children: React.ReactNode }) {
  return (
    <div
      ...
      style={{ background: "rgba(0,208,132,0.06)", border: "1px solid rgba(0,208,132,0.15)", color: "#8B8FA8" }}
    >
      {children}
    </div>
  )
