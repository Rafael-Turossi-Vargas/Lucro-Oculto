    ...
    <div className="max-w-xl mx-auto">
      ...
      <div className="mb-8">
        <div className="flex items-center justify-between mb-2">
          <p className="text-xs font-medium" style={{ color: "#8B8FA8" }}>Passo {step + 1} de {TOTAL}</p>
          <p className="text-xs" style={{ color: "#4B4F6A" }}>{Math.round(progress)}% completo</p>
        </div>
      ...
      <div className="rounded-2xl p-8" style={{ background: "#1A1D27", border: "1px solid #2A2D3A" }}>
        ...
        {step === 0 && (
          <div>
            <h2 className="text-xl font-bold mb-1" style={{ color: "#F4F4F5" }}>Qual é o seu tipo de negócio?</h2>
            <p className="text-sm mb-6" style={{ color: "#8B8FA8" }}>Isso nos ajuda a personalizar sua análise.</p>
              ...
              {BUSINESS_TYPES.map((bt) => {
                ...
                return (
                  <button key={bt.value} onClick={() => setForm((p) => ({ ...p, businessType: bt.value }))}
                    ...
                    style={{
                      background: active ? "rgba(0,208,132,0.08)" : "#212435",
                      border: active ? "1px solid #00D084" : "1px solid #2A2D3A",
                    }}>
                    <Icon className="w-5 h-5 shrink-0 mt-0.5" style={{ color: active ? "#00D084" : "#8B8FA8" }} />
                    <div>
                      <p className="text-sm font-medium" style={{ color: active ? "#F4F4F5" : "#F4F4F5" }}>{bt.label}</p>
                      <p className="text-xs" style={{ color: "#8B8FA8" }}>{bt.desc}</p>
                    </div>
                  </button>
                )
              })}
        ...
        {step === 1 && (
          <div>
            <h2 className="text-xl font-bold mb-1" style={{ color: "#F4F4F5" }}>Qual o faturamento mensal médio?</h2>
            <p className="text-sm mb-6" style={{ color: "#8B8FA8" }}>Usado para calibrar os benchmarks da análise.</p>
              ...
              {REVENUES.map((r) => {
                ...
                return (
                  <button key={r} onClick={() => setForm((p) => ({ ...p, monthlyRevenueRange: r }))}
                    ...
                    style={{ background: active ? "rgba(0,208,132,0.08)" : "#212435", border: active ? "1px solid #00D084" : "1px solid #2A2D3A" }}>
                    <div className="w-4 h-4 rounded-full border-2 flex items-center justify-center shrink-0"
                      style={{ borderColor: active ? "#00D084" : "#3D4158" }}>
                      {active && <div className="w-2 h-2 rounded-full" style={{ background: "#00D084" }} />}
                    </div>
                    <span className="text-sm font-medium" style={{ color: "#F4F4F5" }}>{r}</span>
                  </button>
                )
              })}
            </div>
        ...
        {step === 2 && (
          <div>
            <h2 className="text-xl font-bold mb-1" style={{ color: "#F4F4F5" }}>Quantos funcionários você tem?</h2>
            <p className="text-sm mb-6" style={{ color: "#8B8FA8" }}>Incluindo sócios e prestadores de serviço fixos.</p>
            <div className="grid grid-cols-2 gap-3">
              {EMPLOYEES.map((e) => {
                ...
                return (
                  <button key={e} onClick={() => setForm((p) => ({ ...p, employeeCountRange: e }))}
                    ...
                    style={{ background: active ? "rgba(0,208,132,0.08)" : "#212435", border: active ? "1px solid #00D084" : "1px solid #2A2D3A", color: "#F4F4F5" }}>
                    {e}
                  </button>
                )
              })}
        ...
        {step === 3 && (
          <div>
            <h2 className="text-xl font-bold mb-1" style={{ color: "#F4F4F5" }}>Como você controla o financeiro hoje?</h2>
            <p className="text-sm mb-6" style={{ color: "#8B8FA8" }}>Selecione todas as ferramentas que usa.</p>
            <div className="space-y-2">
              {TOOLS.map((t) => {
                ...
                return (
                  <button key={t} onClick={() => toggleArray("financialTools", t)}
                    ...
                    <div className="w-4 h-4 rounded flex items-center justify-center shrink-0"
                      style={{ background: active ? "#00D084" : "transparent", border: active ? "none" : "1.5px solid #3D4158" }}>
                      {active && <span className="text-[10px] font-bold" style={{ color: "#0F1117" }}>✓</span>}
                    </div>
                    <span className="text-sm font-medium" style={{ color: "#F4F4F5" }}>{t}</span>
                  </button>
                )
              })}
            </div>
        ...
        {step === 4 && (
          <div>
            <h2 className="text-xl font-bold mb-1" style={{ color: "#F4F4F5" }}>Qual sua principal dor financeira?</h2>
            <p className="text-sm mb-4" style={{ color: "#8B8FA8" }}>Selecione as que mais representam sua situação atual.</p>
              ...
              {PAINS.map((pain) => {
                ...
                return (
                  <button key={pain} onClick={() => toggleArray("mainPainPoints", pain)}
                    ...
                    style={{ background: active ? "rgba(255,77,79,0.06)" : "#212435", border: active ? "1px solid rgba(255,77,79,0.3)" : "1px solid #2A2D3A" }}>
                    <div className="w-4 h-4 rounded shrink-0 flex items-center justify-center"
                      style={{ background: active ? "#FF4D4F" : "transparent", border: active ? "none" : "1.5px solid #3D4158" }}>
                      {active && <span className="text-[10px] font-bold" style={{ color: "#fff" }}>✓</span>}
                    </div>
                    <span className="text-sm" style={{ color: "#F4F4F5" }}>{pain}</span>
                  </button>
                )
              })}
            </div>

            <p className="text-sm font-semibold mb-3" style={{ color: "#F4F4F5" }}>Objetivo principal:</p>
        ...
        <div className="flex justify-between mt-8">
          <button onClick={() => setStep((s) => Math.max(0, s - 1))} disabled={step === 0}
            className={btnBase} style={{ color: "#8B8FA8", opacity: step === 0 ? 0.3 : 1 }}>
            <ArrowLeft className="w-4 h-4" /> Voltar
          </button>

          {step < TOTAL - 1 ? (
            <button onClick={() => setStep((s) => s + 1)} disabled={!canNext()}
              className={btnBase} style={{ background: canNext() ? "#00D084" : "#212435", color: canNext() ? "#0F1117" : "#4B4F6A" }}>
              Próximo <ArrowRight className="w-4 h-4" />
            </button>
          ) : (
            <button onClick={handleFinish} disabled={!canNext() || loading}
              className={btnBase} style={{ background: "#00D084", color: "#0F1117", opacity: (!canNext() || loading) ? 0.7 : 1 }}>
