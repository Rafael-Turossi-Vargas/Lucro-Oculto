...
function RoleSelect({
  ...
  return (
    <div className="relative">
      ...
      {open && (
        <div className="absolute right-0 top-full mt-1 z-30 bg-[#1A1D27] border border-[#2A2D3A] rounded-xl shadow-xl w-60 overflow-hidden">
          {availableRoles.map(r => {
            const c = ROLE_COLORS[r]
            return (
              <button key={r} type="button" onClick={() => { onChange(r); setOpen(false) }}
                className="w-full flex items-start gap-3 px-4 py-3 hover:bg-[#212435] transition-colors text-left">
                ...
                <div>
                  ...
                  <p className="text-[11px] text-[#4B4F6A] leading-tight mt-0.5">{ROLES[r].description}</p>
                </div>
              </button>
            )
          })}
...
export default function TeamPage() {
    ...
    return (
      <div className="min-h-[60vh] flex items-center justify-center p-6">
        <div className="text-center max-w-md">
          ...
          <p className="text-[#8B8FA8] mb-6">Gerencie até 5 membros na sua equipe com o plano Premium.</p>
  ...
  if (loading) return <div className="px-6 py-8 space-y-4 animate-pulse">{[1,2,3].map(i => <div key={i} className="h-16 rounded-xl bg-[#212435]" />)}</div>
  ...
  return (
    <div className="px-6 py-8 space-y-8">
      ...
      <div className="bg-[#1A1D27] border border-[#2A2D3A] rounded-2xl p-5">
        <p className="text-xs font-semibold text-[#4B4F6A] uppercase tracking-widest mb-3">Funções e permissões</p>
            ...
            return (
              <div key={key} className="flex items-start gap-2.5 p-2.5 rounded-lg border"
                ...
                <div>
                  ...
                  <p className="text-[11px] text-[#8B8FA8] leading-tight">{def.description}</p>
                </div>
              </div>
            )
          })}
      ...
      {canInvite && memberCount < maxMembers && invitableRoles.length > 0 && (
        <div className="bg-[#1A1D27] border border-[#2A2D3A] rounded-2xl p-6">
          <h2 className="text-base font-semibold text-[#F4F4F5] mb-4 flex items-center gap-2">
          ...
          <form onSubmit={handleInvite} className="space-y-3">
            <div className="flex gap-3">
              <input type="email" value={inviteEmail} onChange={e => setInviteEmail(e.target.value)}
                ...
                className="flex-1 bg-[#212435] border border-[#2A2D3A] rounded-lg px-4 py-2.5 text-sm text-[#F4F4F5] placeholder-[#4B4F6A] focus:outline-none focus:border-[#A855F7] transition-colors" />
              <RoleSelect value={inviteRole} onChange={v => setInviteRole(v as Role)} availableRoles={invitableRoles} />
            </div>
            <div className="flex items-center justify-between gap-3">
              <p className="text-xs text-[#4B4F6A] flex-1">{ROLES[inviteRole]?.description}</p>
              ...
              <div key={m.id} className="flex items-center gap-4 px-6 py-4">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2">
                    <p className="text-sm font-medium text-[#F4F4F5] truncate">{m.user.name ?? "—"}</p>
                    {isMe && <span className="text-[10px] text-[#4B4F6A]">(você)</span>}
                  </div>
                  <p className="text-xs text-[#8B8FA8] truncate">{m.user.email}</p>
                </div>
                {canEdit ? (
                  <div className="flex items-center gap-1.5">
                    {isEditing
                      ? <span className="text-xs text-[#4B4F6A] px-2">Salvando...</span>
                      : <><RoleSelect value={m.role} onChange={nr => handleChangeRole(m.user.id, nr)} availableRoles={invitableRoles} disabled={isEditing} /><Pencil className="w-3 h-3 text-[#4B4F6A]" /></>
                    }
                  </div>
                ) : (
                ...
                {canRemove && !isMe && !isOwnerRow && (
                  <button onClick={() => handleRemove(m.user.id)} disabled={removingId === m.user.id}
                    className="p-1.5 rounded-lg text-[#4B4F6A] hover:text-[#FF4D4F] hover:bg-[#FF4D4F]/10 transition-colors disabled:opacity-50" title="Remover membro">
            ...
            {data?.invites.map(inv => {
              ...
              return (
                <div key={inv.id} className="flex items-center gap-4 px-6 py-4">
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-medium text-[#F4F4F5] truncate">{inv.email}</p>
                    <p className="text-xs text-[#8B8FA8]">Expira em {daysLeft} dia{daysLeft !== 1 ? "s" : ""}</p>
                  </div>
                  ...
                  {canInvite && (
                    <button onClick={() => handleCancelInvite(inv.token)} disabled={cancellingToken === inv.token}
                      className="p-1.5 rounded-lg text-[#4B4F6A] hover:text-[#FF4D4F] hover:bg-[#FF4D4F]/10 transition-colors disabled:opacity-50" title="Cancelar convite">
                      <X className="w-4 h-4" />
                    </button>
                  )}
                </div>
